                        ...
                        messages=[
                            {
                                ...
                                "content": (
                                    ...
                                    "4. For layers that support dropout, include the parameter `dropout_rate` within the "
                                    "layer's key=value pairs.\n"
                                    ...
                                    "8. Do not include any extra text or JSON outside the specified format. If multiple experiments "
                                    "are generated, each should be on a separate line.\n\n"
                                    ...
                                    "**Example (Varying Optimization Fields with Adam):**\n"
                                    "```\n"
                                    "based_on_id:142;loss_fn:Cross Entropy Loss;optimization:Adam;min_delta:0.0005;patience:3;"
                                    "normalization:None;batch_size:64;weight_decay:0.0002;learning_rate:0.0005;"
                                    "epochs:12;layers:"
                                    "activation_fn=None,input=(32,32,3),dropout_rate=0.0,layer_fields=input_shape=(32,32,3),"
                                    "layer_type=Input,output=(32,32,3)|"
                                    "activation_fn=ReLU,input=(32,32,3),dropout_rate=0.25,layer_fields=in_channels=3+kernel_size=3+"
                                    "out_channels=128+padding=1+stride=1,layer_type=CNN,weight_initiations=Xavier Initialization,"
                                    "output=(30,30,128)|"
                                    "activation_fn=None,input=(30,30,128),dropout_rate=0.0,layer_fields=pool_size=2+pool_type=max+"
                                    "stride=2,layer_type=Pooling,output=(15,15,128),weight_initiations=None|"
                                    "activation_fn=ReLU,input=28800,dropout_rate=0.3,layer_fields=units=512,layer_type=Dense,"
                                    "weight_initiations=Xavier Initialization,output=512|"
                                    "activation_fn=Softmax,input=512,dropout_rate=0.0,layer_fields=output_shape=10,layer_type=Output,"
                                    "output=10;optimization_fields:beta1=0.85+beta2=0.995+epsilon=1e-07\n"
                                    "```\n\n"
                                    ...
                                    "- Remember to include the `optimization_fields` line for each experiment, even if only one parameter changes.\n"
